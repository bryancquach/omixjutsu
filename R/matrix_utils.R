...
exceeds_min_cutoff <- function(metrics_table,
  ...
  if (use_skewed_iqr) {
    output$cutoff <- adjboxStats(metric_vec, coef = coef, do.conf = F, do.out = F)$fence[1]
  }
  if (include_equals) {
...
below_max_cutoff <- function(metrics_table,
  ...
  if (use_skewed_iqr) {
    output$cutoff <- adjboxStats(metric_vec, coef = coef, do.conf = F, do.out = F)$fence[2]
  }
  if (include_equals) {
    output$keep <- metric_vec <= output$cutoff
  } else {
...
select_replicates <- function(data, id, metric, max_replicates = 1, decreasing = T) {
  ...
  data_tmp <- data.frame(row_index = 1:nrow(data), data)
